package com.action.rest_mysql_02.controller

import com.action.rest_mysql_02.model.Post
import com.action.rest_mysql_02.repository.PostRepository
...
class PostController(private val postRepository: PostRepository) {
    ...
    fun updatePostById(@PathVariable(value = "id") postId: Long,
        ...
        return postRepository.findById(postId).map {
            existingPost -> val updatePost: Post = existingPost
                            .copy(title = newPost.title, content = newPost.content)

            ResponseEntity.ok().body(postRepository.save(updatePost))
            }.orElse(ResponseEntity.notFound().build())
    }
